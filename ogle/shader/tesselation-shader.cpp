...
"    vec4 vertexNDS = viewProjectionMatrix * vertexWS;\n"
...
"     float d = (max - clamp(distance(v,cameraPosition), min, max))/(max-min);\n"
"     return clamp( d*64.0*factor, 1, 64);\n"
"}\n";
...
const string tessWorldPos(GLint i, bool useDisplacement)
{
  stringstream s;
  if(useDisplacement) {
    s << "  gl_in["<<i<<"].gl_Position + vec4(tcs_displacement["<<i<<"],0.0);" << endl;
  } else {
...
{
  addUniform( GLSLUniform( "vec3", "cameraPosition" ) );
  addUniform( GLSLUniform( "vec2", "viewport" ) );
  addUniform( GLSLUniform( "float", "lodFactor" ) );
  ...
  if(useDisplacement_) addInput(GLSLTransfer("vec3", "tcs_displacement"));
}
...
string QuadTesselationControl::code() const
{
  ...
  case TESS_LOD_EDGE_SCREEN_DISTANCE:
    s << "    vec2 ss0 = deviceToScreenSpace(nds0, viewport);" << endl;
    s << "    vec2 ss1 = deviceToScreenSpace(nds1, viewport);" << endl;
    s << "    vec2 ss2 = deviceToScreenSpace(nds2, viewport);" << endl;
    s << "    vec2 ss3 = deviceToScreenSpace(nds3, viewport);" << endl;
    s << "    float e0 = metricSreenDistance(ss1.xy, ss2.xy, lodFactor);" << endl;
    s << "    float e1 = metricSreenDistance(ss0.xy, ss1.xy, lodFactor);" << endl;
    s << "    float e2 = metricSreenDistance(ss3.xy, ss0.xy, lodFactor);" << endl;
    s << "    float e3 = metricSreenDistance(ss2.xy, ss3.xy, lodFactor);" << endl;
    break;
  case TESS_LOD_EDGE_DEVICE_DISTANCE:
    s << "    float e0 = metricDeviceDistance(nds1.xy, nds2.xy, lodFactor);" << endl;
    s << "    float e1 = metricDeviceDistance(nds0.xy, nds1.xy, lodFactor);" << endl;
    s << "    float e2 = metricDeviceDistance(nds3.xy, nds0.xy, lodFactor);" << endl;
    s << "    float e3 = metricDeviceDistance(nds2.xy, nds3.xy, lodFactor);" << endl;
    break;
  case TESS_LOD_CAMERA_DISTANCE_INVERSE:
    s << "    float e0 = metricCameraDistance(ws0.xyz, lodFactor);" << endl;
    s << "    float e1 = metricCameraDistance(ws1.xyz, lodFactor);" << endl;
    s << "    float e2 = metricCameraDistance(ws2.xyz, lodFactor);" << endl;
    s << "    float e3 = metricCameraDistance(ws3.xyz, lodFactor);" << endl;
    break;
  }
...
string TriangleTesselationControl::code() const
{
  ...
  case TESS_LOD_EDGE_SCREEN_DISTANCE:
    ...
    s << "    float e0 = metricSreenDistance(ss1.xy, ss2.xy, lodFactor);" << endl;
    s << "    float e1 = metricSreenDistance(ss0.xy, ss1.xy, lodFactor);" << endl;
    s << "    float e2 = metricSreenDistance(ss2.xy, ss0.xy, lodFactor);" << endl;
    break;
  case TESS_LOD_EDGE_DEVICE_DISTANCE:
    s << "    float e0 = metricDeviceDistance(nds1.xy, nds2.xy, lodFactor);" << endl;
    s << "    float e1 = metricDeviceDistance(nds0.xy, nds1.xy, lodFactor);" << endl;
    s << "    float e2 = metricDeviceDistance(nds2.xy, nds0.xy, lodFactor);" << endl;
    break;
  case TESS_LOD_CAMERA_DISTANCE_INVERSE:
    s << "    float e0 = metricCameraDistance(ws0.xyz, lodFactor);" << endl;
    s << "    float e1 = metricCameraDistance(ws1.xyz, lodFactor);" << endl;
