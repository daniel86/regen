...
FurVolume::FurVolume(Vec3ui size)
: Texture3D()
{
  pixels_ = new char[size.x*size.y*size.z];
  ...
  bind();
  set_filter(GL_LINEAR, GL_LINEAR);
  set_wrappingU(GL_REPEAT);
  set_wrappingV(GL_REPEAT);
  set_wrappingW(GL_REPEAT);

  clear();

  addMapTo(MAP_TO_VOLUME_SLICE);
}
...
void FurVolume::applyUniformNoise(GLuint hairCount, GLuint seedPos)
{
  float lengthInverse;
  int density, layerPos, index;
  int texSize = width_*height_;

  for(GLuint layer=0; layer<numTextures_; layer++)
  {
    lengthInverse = 1.0f - float(layer)/numTextures_;
    density = int(hairCount*lengthInverse);
    layerPos = layer*texSize;

    srand(seedPos);

    for(int i=0; i<density; i++)
    {
      index = layerPos + (int)rnd(0, (float)texSize);
      pixels_[index] = (int)(lengthInverse*255.0f);
    }
  }
}

void FurVolume::clear()
{
  GLuint size = width_*height_*numTextures_;
  for(GLuint i=0; i<size; ++i) pixels_[i] = 0;
}
