...
namespace ogle {
class ShadowMapNew : public ShaderInputState
{
public:
  struct Config {
    GLuint size;
...
protected:
  ...
  void (ShadowMapNew::*update_)();
  void updateDirectional();
  void updatePoint();
  void updateSpot();

  void (ShadowMapNew::*computeDepth_)(RenderState *rs);
...
string ShadowMapNew::shadowFilterMode(ShadowMapNew::FilterMode f)
{
  switch(f) {
  case ShadowMapNew::FILTERING_NONE: return "Single";
  case ShadowMapNew::FILTERING_PCF_GAUSSIAN: return "Gaussian";
  case ShadowMapNew::FILTERING_VSM: return "VSM";
  }
  return "Single";
}
GLboolean ShadowMapNew::useShadowMoments(ShadowMapNew::FilterMode f)
{
  switch(f) {
  case ShadowMapNew::FILTERING_VSM:
    return GL_TRUE;
  default:
    return GL_FALSE;
  }
}
GLboolean ShadowMapNew::useShadowSampler(ShadowMapNew::FilterMode f)
{
  switch(f) {
  case ShadowMapNew::FILTERING_VSM:
    return GL_FALSE;
  ...
  cfg_(cfg)
{
  ...
  case Light::DIRECTIONAL:
    ...
    update_ = &ShadowMapNew::updateDirectional;
    computeDepth_ = &ShadowMapNew::computeDirectionalDepth;
    break;
  case Light::POINT:
    cfg_.textureTarget = GL_TEXTURE_CUBE_MAP;
    cfg_.numLayer = 6;
    update_ = &ShadowMapNew::updatePoint;
    computeDepth_ = &ShadowMapNew::computePointDepth;
    break;
  case Light::SPOT:
    cfg_.textureTarget = GL_TEXTURE_2D;
    cfg_.numLayer = 1;
    update_ = &ShadowMapNew::updateSpot;
    computeDepth_ = &ShadowMapNew::computeSpotDepth;
    break;
  }

  // TODO: SHADOW: no inverse matrices provided
  depthFBO_ = ref_ptr<FrameBufferObject>::manage( new FrameBufferObject(
      cfg.size,cfg.size,cfg.numLayer,
      cfg_.textureTarget,cfg.depthFormat,cfg.depthType));
  ...
  setInput(ref_ptr<ShaderInput>::cast(shadowNear_));
...
void ShadowMapNew::setPolygonOffset(GLfloat factor, GLfloat units)
{
...
void ShadowMapNew::setCullFrontFaces(GLboolean v)
{
  if(cullState_.get()) {
...
void ShadowMapNew::set_depthFormat(GLenum f)
{
  cfg_.depthFormat = f;
  depthTexture_->bind();
...
void ShadowMapNew::set_depthType(GLenum t)
{
  cfg_.depthType = t;
  depthTexture_->bind();
...
void ShadowMapNew::set_depthSize(GLuint shadowMapSize)
{
...
void ShadowMapNew::set_shadowLayer(GLuint numLayer)
{
...
void ShadowMapNew::set_splitWeight(GLdouble splitWeight)
{
  cfg_.splitWeight = splitWeight;
}

void ShadowMapNew::set_isCubeFaceVisible(GLenum face, GLboolean visible)
{
...
void ShadowMapNew::updateDirectional()
{
...
void ShadowMapNew::updatePoint()
{
  ...
  projectionMatrix_[0] = Mat4f::projectionMatrix(
      90.0, 1.0f, shadowNear()->getVertex1f(0), far);
...
void ShadowMapNew::computeDirectionalDepth(RenderState *rs)
{
...
void ShadowMapNew::computePointDepth(RenderState *rs)
{
...
void ShadowMapNew::computeSpotDepth(RenderState *rs)
{
  sceneCamera_->position()->pushData(light_->position()->dataPtr());
  sceneCamera_->view()->pushData((byte*) viewMatrix_[0].x);
  sceneCamera_->projection()->pushData((byte*) projectionMatrix_[0].x);
...
void ShadowMapNew::setComputeMoments()
{
...
void ShadowMapNew::createBlurFilter(
    GLuint size, GLfloat sigma,
    GLboolean downsampleTwice)
{
...
void ShadowMapNew::addCaster(const ref_ptr<StateNode> &caster)
{
  caster_.push_back(caster);
}
void ShadowMapNew::removeCaster(StateNode *caster)
{
...
void ShadowMapNew::traverse(RenderState *rs)
{
  for(list< ref_ptr<StateNode> >::iterator
      it=caster_.begin(); it!=caster_.end(); ++it)
  {
...
void ShadowMapNew::update(RenderState *rs, GLdouble dt)
{
  (this->*update_)();
...
const ref_ptr<Texture>& ShadowMapNew::shadowDepth() const
{ return depthTexture_; }

const ref_ptr<Texture>& ShadowMapNew::shadowMomentsUnfiltered() const
{ return momentsTexture_; }
const ref_ptr<Texture>& ShadowMapNew::shadowMoments() const
{ return momentsFilter_->output(); }
const ref_ptr<FilterSequence>& ShadowMapNew::momentsFilter() const
{ return momentsFilter_; }
const ref_ptr<ShaderInput1f>& ShadowMapNew::momentsBlurSize() const
{ return momentsBlurSize_; }
const ref_ptr<ShaderInput1f>& ShadowMapNew::momentsBlurSigma() const
{ return momentsBlurSigma_; }

const ref_ptr<ShaderInputMat4>& ShadowMapNew::shadowMat() const
{ return shadowMat_; }
const ref_ptr<ShaderInput1f>& ShadowMapNew::shadowFar() const
{ return shadowFar_; }
const ref_ptr<ShaderInput1f>& ShadowMapNew::shadowNear() const
{ return shadowNear_; }
const ref_ptr<ShaderInput1f>& ShadowMapNew::shadowMapSize() const
