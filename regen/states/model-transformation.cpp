...
static GLuint transformMatrixPlane(
	...
	if (generator.instanceData.empty()) {
		...
		auto matrices = matrixInput->mapClientData<Mat4f>(ServerAccessMode::WRITE);
	...
	for (auto &child: input.getChildren()) {
		...
		if (child->getCategory() == "set") {
			...
			if (mode == "plane") {
				...
				auto matrices = tf->modelMat()->mapClientData<Mat4f>(ServerAccessMode::WRITE);
			...
			auto v_modelMat = modelMat->mapClientData<Mat4f>(ServerAccessMode::WRITE);
			auto v_modelOffset = modelOffset->mapClientData<Vec4f>(ServerAccessMode::WRITE);
...
ModelTransformation::load(LoadingContext &ctx, scene::SceneInputNode &input, const ref_ptr<State> &state) {
	...
	if (input.getValue<std::string>("gpu-usage", "READ") == "WRITE") {
		transform->modelMat()->set_gpuUsage(ServerAccessMode::WRITE);
		transform->modelOffset()->set_gpuUsage(ServerAccessMode::WRITE);
	} else {
		transform->modelMat()->set_gpuUsage(ServerAccessMode::READ);
		transform->modelOffset()->set_gpuUsage(ServerAccessMode::READ);
	}
	...
	if (isInstanced && numInstances > 1) {
		...
		if (transform->hasModelMat()) {
			...
			auto matrices = modelMat->mapClientData<Mat4f>(ServerAccessMode::WRITE);
			...
			auto offsets = modelOffset->mapClientData<Vec4f>(ServerAccessMode::WRITE);
