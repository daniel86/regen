...
{
  Mat4f *shadowMatrices = (Mat4f*)shadowMat_->clientDataPtr();
  ...
  if(projectionStamp_ != sceneCamera_->projection()->stamp())
  {
    ...
    GLfloat *farValues = (GLfloat*)shadowFar_->clientDataPtr();
    GLfloat *nearValues = (GLfloat*)shadowNear_->clientDataPtr();
...
{
  sceneCamera_->position()->pushClientData((byte*)&Vec3f::zero().x);
  sceneCamera_->view()->pushClientData((byte*)viewMatrix_[0].x);
  for(register GLuint i=0; i<cfg_.numLayer; ++i)
  {
    ...
    sceneCamera_->projection()->pushClientData((byte*)projectionMatrix_[i].x);
    sceneCamera_->viewProjection()->pushClientData((byte*)viewProjectionMatrix_[i].x);

    traverse(rs);

    sceneCamera_->viewProjection()->popClientData();
    sceneCamera_->projection()->popClientData();
  }
  sceneCamera_->view()->popClientData();
  sceneCamera_->position()->popClientData();
}

void ShadowMap::computePointDepth(RenderState *rs)
{
  sceneCamera_->position()->pushClientData(light_->position()->clientDataPtr());
  sceneCamera_->projection()->pushClientData((byte*)projectionMatrix_[0].x);
  for(register GLuint i=0; i<6; ++i)
  {
    if(!isCubeFaceVisible_[i]) { continue; }
    ...
    sceneCamera_->view()->pushClientData((byte*)viewMatrix_[i].x);
    sceneCamera_->viewProjection()->pushClientData((byte*)viewProjectionMatrix_[i].x);
...
void ShadowMap::computeSpotDepth(RenderState *rs)
{
  ...
  sceneCamera_->view()->pushClientData((byte*) viewMatrix_[0].x);
  sceneCamera_->projection()->pushClientData((byte*) projectionMatrix_[0].x);
  sceneCamera_->viewProjection()->pushClientData((byte*) viewProjectionMatrix_[0].x);
...
void ShadowMap::setComputeMoments()
{
  ...
  momentsQuad_->updateVAO(rs, cfg.cfg(), momentsCompute_->shader());
