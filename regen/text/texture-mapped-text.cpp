...
void TextureMappedText::updateAttributes(Alignment alignment, GLfloat maxLineWidth) {
	...
	auto v_pos = posAttribute_->mapClientData<Vec3f>(ServerAccessMode::WRITE);
	auto v_texco = texcoAttribute_->mapClientData<Vec3f>(ServerAccessMode::WRITE);
	auto v_nor = norAttribute_->mapClientData<Vec3f>(ServerAccessMode::WRITE);
