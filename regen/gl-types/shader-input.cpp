...
void ShaderInput::writeVertex(GLuint index, const byte *data) {
	...
	if (isVertexAttribute_) {
		auto mapped = mapClientDataRaw(ClientMappingMode::WRITE, index * elementSize_, elementSize_);
		std::memcpy(mapped.w, data, elementSize_);
	} else {
		auto arrayElementSize = dataTypeBytes_ * valsPerElement_;
		auto mapped = mapClientDataRaw(ClientMappingMode::WRITE, index * arrayElementSize, arrayElementSize);
...
void ShaderInput::writeServerData(GLuint index) const {
	...
	// TODO use ROI class here
	auto mappedClientData = clientBuffer_.mapRange(ClientMappingMode::READ, 0, inputSize_);
...
void ShaderInput::readServerData() {
	...
	clientBuffer_.unmapRange(ClientMappingMode::WRITE, 0, inputSize_, mappedClientData.w_index);
}
...
ShaderInput1f::ShaderInput1f(
	...
	enableInput_ = [this](GLint loc) {
		auto mapped = mapClientDataRaw(ClientMappingMode::READ);
