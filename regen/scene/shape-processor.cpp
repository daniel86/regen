...
createConvexHull(SceneInputNode &input, const ref_ptr<Mesh> &mesh) {
	...
	auto v_pos = pos->mapClientData<btScalar>(ClientMappingMode::READ);
...
createTriangleMesh(SceneInputNode &input, const ref_ptr<Mesh> &mesh) {
	...
	auto v_pos = pos->mapClientDataRaw(ClientMappingMode::READ);
	auto indices_data = indices->mapClientDataRaw(ClientMappingMode::READ);
