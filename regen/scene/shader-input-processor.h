...
namespace regen {
	...
	public:
		...
		void animate(GLdouble dt) override {
			auto mapped = mapClientVertex<float>(ClientMappingMode::READ | ClientMappingMode::WRITE, 0);
	...
	namespace scene {
		...
		public:
			...
			static void setInput(SceneInputNode &input, ShaderInput *shaderInput, unsigned int count) {
				auto v_values = shaderInput->mapClientData<T>(ClientMappingMode::WRITE | ClientMappingMode::READ);
			...
			static ref_ptr<U> createShaderInputTyped(
				...
				if (input.getValue<std::string>("gpu-usage", "READ") == "WRITE") {
					v->set_gpuUsage(ClientMappingMode::WRITE);
				} else {
					v->set_gpuUsage(ClientMappingMode::READ);
				}
				...
				if (isInstanced || isAttribute) {
					auto values = v->mapClientDataRaw(ClientMappingMode::WRITE);
